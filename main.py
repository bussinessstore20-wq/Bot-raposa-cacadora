...
# ==========================================
# Lê da variável de ambiente no Render; se não existir, usa o token direto
...
def enviar_oferta(foto_url, legenda, link_afiliado):
    ...
    try:
        ...
        print(f"⚠️ Erro no envio para a API do Telegram: {e}")
        return False
    ...
    for link in links_unicos:
        ...
        try:
            ...
            if enviar_oferta(foto_url, legenda, link):
                ...
                print(f"✅ [POSTADO {postados_nesta_rodada}/{quantidade_maxima}] {titulo[:30]}...")
            ...
            if "result" in res:
                for update in res["result"]:
                    ...
                    if "message" in update and "web_app_data" in update["message"]:
                        ...
                        # Executa o scraping em uma Thread separada para não travar o loop de escuta
                        t_postagem = threading.Thread(
                            target=processar_e_postar_vitrine,
                            args=(link, quantidade, intervalo)
                        )
                        t_postagem.daemon = True
                        t_postagem.start()
...
if __name__ == "__main__":
    # Inicia o servidor Web (Flask) para o Render
    t_web = threading.Thread(target=iniciar_servidor_web)
    t_web.daemon = True
    t_web.start()

    # Inicia a escuta de ordens do Mini App
    escutar_mini_app()
